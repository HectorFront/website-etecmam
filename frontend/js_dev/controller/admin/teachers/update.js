const updateProf = (id) => {
    ...
    let idURL, PermissionidUser = localStorage.getItem(btoa('idUser'));
        ...
        $.get(`${ENV.API_URL}/get/user/${PermissionidUser}`)
            .done(res => {
                ...
                if (PermissionToken === res.webtoken && hashValidade === generateHash(res.email)) {
                    let nomeProf = $(`#name${id}`).val();
                    let emailProf = $(`#email${id}`).val();
                    ...
                    if (!nomeProf || !emailProf) {
                        ...
                        swal({
                            ...
                            text: "Se você atualizar este professor a lista será atualizada automaticamente!",
                            icon: "warning",
                            buttons: true,
                            dangerMode: true,
                        })
                            .then((willUpdate) => {
                                if (willUpdate) {
                                    ...
                                    $.ajax({
                                        url: `${ENV.API_URL}/put/professor/${id}`,
                                        ...
                                        $.get(`${ENV.API_URL}/get/user/${(idURL)}`)
                                            .done(res => {
                                                ...
                                                let data = {
                                                    ...
                                                    cargo: res.cargo,
                                                    atividade: 'Atualizou em <a href="/Professores-ADMIN">Professores</a>'
                                                }
                                        ...
                                        swal("Professor atualizado!", "Clique em ok para sair", "success").then(() => {
                                            let loading = `
                                            <div class="load">
                                                <div class="lds-ring">
                                                    <div></div>
                                                            ...
                                                            `<div id="item${resp.id}" class="data-teacher">
                                                                <div class="info-prof">
                                                                        <p> <span class="professor">${resp.nome}</span>
                                                                            ...
                                                                            <button onclick="deleteModal(${resp.id})" class="edit-prof"><i class="fa fa-trash"></i></i></button>
                                                                        </p>
                                                                </div>
                                                                <div style="margin-left: 10px; margin-top: 25px; margin-right: 10px;" id="edit-form${resp.id}" class="edit-form">
                                                                    <form>
                                        ...
                                        swal("Professor não atualizado, por favor contatar com os programadores!", "Clique em ok para sair", "error").then(() => {
