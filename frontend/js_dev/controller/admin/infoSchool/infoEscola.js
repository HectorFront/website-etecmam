    ...
    $.get(`${ENV.API_URL}/get/user/${PermissionidUser}`)
      .done(res => {
        ...
        if (PermissionToken === res.webtoken && hashValidade === generateHash(res.email)) {
          let alunos = $('#alunos').val();
          let tradicao = $('#tradicao').val();
          let quantidadecursos = $('#quantidadecursos').val();

          swal({
            title: "Deseja mesmo atualizar?",
            text: "Se você atualizar esta informação a página inicial será atualizada automaticamente!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
          }).then((results) => {
            if (results) {
              ...
              $.ajax({
                url: `${ENV.API_URL}/put/info/escola`,
                ...
                $.get(`${ENV.API_URL}/get/user/${(idURL)}`)
                  .done(res => {
                    ...
                    let data = {
                      ...
                      cargo: res.cargo,
                      atividade: 'Atualizou informações da escola'
                    }
                ...
                swal("Informações editadas!", "Clique em ok para sair", "success")
                  .then(() => {
                    $.get(`${ENV.API_URL}/get/info/escola`)
                      .done(res => {
                        ...
                        console.log({ error: 'Error in request info escola' })
                      });
                  });
              }).fail(() => {
                swal("Falha na edição, por favor contatar com os programadores!", "Clique em ok para sair", "error").then(() => {
