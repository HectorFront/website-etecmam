...
$('#form-publication').on('submit', function (event) {
    ...
    var id = localStorage.getItem(btoa('idUser'));
    ...
    var registerLogPublication = function registerLogPublication() {
    ...
    if (PermissionidUser === PermissionidDefault) {
        ...
        $.get(ENV.API_URL + '/get/user/' + PermissionidUser).done(function (res) {
            ...
            if (PermissionToken === res.webtoken && hashValidade === generateHash(res.email)) {
                var titulo = $('#titleevent').val();
                var assunto = $('#assuntoevent').val();
                var descricao = $('#descevent').val();
                var videoyt = $('#video-yt').val();

                //file image
                var newFileName = void 0;
                var fileValue = $("#image-publication")[0].files[0];
                newFileName = fileValue ? '' + randomHash() + fileValue.name : null;
                var form = new FormData();
                fileValue && form.append('file', fileValue, newFileName);

                //file video
                var newVideoName = void 0;
                var videoValue = $("#video-publication")[0].files[0];
                ...
                if (!titulo || !descricao) {
                    ...
                    $.get(ENV.API_URL + '/get/user/' + id).done(function (res) {
                        ...
                        $.ajax({
                            ...
                            swal("Erro ao publicar, por favor contatar com os programadores!", "Clique em ok para sair", "error").then(function () {
